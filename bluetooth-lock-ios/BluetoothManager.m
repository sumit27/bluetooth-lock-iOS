...
- (void)peripheralManagerDidUpdateState:(CBPeripheralManager *)peripheral{
    ...
    switch (peripheral.state) {
        case CBPeripheralManagerStatePoweredOn:{
            ...
            NSLog(@"%lu",characteristic2.properties);
...
- (NSData *)getNextData
{
    NSData *data;
    if ([mainData length]>19) {
        float datarest = [mainData length]-20;
        data = [mainData subdataWithRange:NSRangeFromString(@"{0,20}")];
        range = [NSString stringWithFormat:@"{20,%f}",datarest];
    }else{
        float datarest = [mainData length];
        range = [NSString stringWithFormat:@"{0,%f}",datarest];
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral didReceiveWriteRequests:(NSArray *)requests{
    for (CBATTRequest *aReq in requests){
        NSString *text;
        text = [text stringByAppendingString:[[NSString alloc]initWithData:aReq.value encoding:NSUTF8StringEncoding]];
